...
namespace Fungus
{
    ...
    public class DialogInput : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            if (writer != null && currentStandaloneInputModule != null)
            {
                // 【防御性修改】：增加对 submitButton 和 cancelButton 的空值与空字符串判断，防止空指针异常
                bool submitPressed = !string.IsNullOrEmpty(currentStandaloneInputModule.submitButton) && 
                                     Input.GetButtonDown(currentStandaloneInputModule.submitButton);

                bool cancelPressed = cancelEnabled && 
                                     !string.IsNullOrEmpty(currentStandaloneInputModule.cancelButton) && 
                                     Input.GetButton(currentStandaloneInputModule.cancelButton);

                if (submitPressed || cancelPressed)
                {
                    SetNextLineFlag();
                }
            }
